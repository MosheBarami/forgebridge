...
and ruff, plus the gate self-tests under `scripts/` and 206 Luau plugin tests run by hand.
...
tree. Two can — the `protocol` suite's, and the plugin's 206 Luau tests — and
...
59 tests are green. `M08` is done too, so the cross-language drift gate the `M07` row used to
...
| # | Milestone | Status | Definition of done |
|---|---|---|---|
| M07 ✅ | `packages/protocol` — Zod schemas for ChangeSet/Run/Link/Apply | NEW | Schemas frozen; TS types are `z.infer` of them, so that projection cannot drift by construction; 59 tests green. Every other projection of these schemas is generated from them by `M08`, never hand-written |
...
| M41 | Test strategy: Vitest units + Playwright E2E + Luau plugin tests | PART | Plugin tests exist: 206 Luau tests in `plugin/tests/`, run by hand with `luau tests/run.luau`. Still to add: those tests in CI (needs a pinned Luau toolchain — see the TODO in `.github/workflows/ci.yml`) and E2E of the full apply/rollback loop |
