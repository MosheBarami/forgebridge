...
| # | Milestone | Status | Definition of done |
|---|---|---|---|
| M07 ✅ | `packages/protocol` — Zod schemas for ChangeSet/Run/Link/Apply | NEW | Schemas frozen; TS types are `z.infer` of them, so that projection cannot drift by construction; 50 tests green. Every other projection of these schemas is generated from them by `M08`, never hand-written |
...
| M11 PART | Journal + inverse operations + `rollback` | NEW | `JournalEntry` was already in the frozen protocol and had no route; `packages/daemon/src/rollback.ts` gives it one, so the inverse operations the plugin captures now leave the Studio session that captured them and a closed Studio is no longer the end of the road back. The daemon orders a reversal, checks each inverse against the operation it reverses, and treats a `restoreSubtree` payload as an opaque blob — it stores and returns it without parsing, so a Roblox format change still needs no daemon release. 50 tests in `packages/daemon/test/rollback.test.ts`. Still `PART`, and the row's own wording says why: **the tree is not yet restored byte-for-byte in a test**. `POST /v1/journal/:id/rollback` dispatches to a paired Studio session and answers `202 dispatched`; every surface above it says "dispatched" rather than "rolled back" because that is all that is proven. Completion has no shape on the wire at all — `RollbackResult` is a local stand-in for a genuine protocol gap, and closing it needs `packages/protocol` unfrozen |
