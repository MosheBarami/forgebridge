...
/**
 * Envelope authentication for the local transport.
 *
 * The daemon runs on the user's own machine, so confidentiality against the
 * transport is not the problem here — *authenticity* is. Any process on the box
 * can open a socket to 127.0.0.1, and a web page the user has open can be made
 * to POST to it. The MAC is what separates "the Studio session we paired with"
 * from "anything else that found the port" (ADR-014 v1: authenticated, not
 * encrypted; the same scheme the relay uses, so the plugin has one code path).
 */
 ...
 * independently — the daemon in JS, the plugin in Luau, where table iteration
...
export function sealEnvelope(
  ...
  return {
    ...
    nonce: input.nonce,
    payload,
    // TODO(M19): `relay-e2e` seals the payload with ChaCha20-Poly1305 and sets
    // this true. The local daemon never encrypts — there is no untrusted hop.
    encrypted: false,
 ...
 * already been read and could already be stale. A caller that owns the
 * watermark (the daemon) omits it and claims the nonce atomically through
 * `DaemonStore.tryAdvanceInboundNonce` instead; a caller that has a watermark
 * in hand and only wants a cheap early reject may pass it.
 */
export function openEnvelope(
  ...
  if (envelope.linkId !== context.linkId) {
    // Cross-link injection: a valid envelope for someone else's link.
  ...
  if (envelope.encrypted) {
    throw new ForgeBridgeError(
      'invalid_request',
      'this transport does not accept encrypted payloads',
      'The local daemon has no untrusted hop; send the payload as canonical JSON.',
    );
  }
